...
public class MasterStoreFile extends AsyncTask<Void, Void, Void> {
    ...
    public MasterStoreFile(int store_number, ArrayList<FileDTO> dtos){
    ...
    public MasterStoreFile(ArrayList<FileDTO> dtos) {
        this.dtos = dtos;
    }
...
// doInBackground 끝난후에 실행하는 부분
    ...
    private FileDTO readMessage(JsonReader reader) throws IOException {
        ...
        while (reader.hasNext()) {
            ...
            if (readStr.equals("file_id")) {
                file_id = reader.nextInt();
            } else if (readStr.equals("store_number")) {
                store_number = reader.nextInt();
            } else if (readStr.equals("rank")) {
                rank = reader.nextInt();
            } else if (readStr.equals("imgpath")) {
                imgpath =reader.nextString();
            } else if (readStr.equals("imgname")) {
                imgname =reader.nextString();
            } else {
                reader.skipValue();
            }
        }
        reader.endObject();
        return new FileDTO(file_id, store_number, imgpath, imgname, rank);
